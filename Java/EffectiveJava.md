...
### :label: Item 87 - "커스텀 직렬화 형태를 고려해보라"
    ...
    클래스를 직렬화하기로 했다면 어떤 직렬화 형태를 사용할 지 심사숙고하자
...
- readResolve 메소드의 접근성
    - final 클래스에서는 readResolve 메소드는 private이어야 한다.

<br>
<br>

### :label: Item 90 - "직렬화된 인스턴스 대신 직렬화 프록시 사용을 검토하라"

    직렬화를 구현해야 하는 경우, 언어의 정상 메커니즘인 생성자 이외의 방법으로 인스턴스를 생성에 따른 버그와 보안 문제가 일어날 가능성이 커진다.
    직렬화 프록시 패턴을 사용하면 이 위험을 크게 줄여줄 수 있다.
...
- **직렬화 프록시 패턴 구현 방법**
    ...
    - 중첩 클래스의 생성자는 단 하나여야 하고, 바깥 클래스를 매개변수로 받아야 한다.
        ...
        - 일관성 검사난 방어적 복사도 필요 없음
    - 바깥 클래스와 직렬화 프록시 모두 Serializable을 구현한다고 선언
    - 바깥 클래스에 writeReplace 메소드 추가(범용적으로 동일하게 작성하면 된다.)
        ```java
        private Object writeReplace() {
            return new SerializationProxy(this);
        }
        ```
        - 이 메소드는 자바의 직렬화 시스템이 바깥 클래스의 인스턴스 대신 SerializationProxy의 인스턴스를 반환하게 하는 역할
        - 직렬화가 이뤄지기 전 바깥 클래스의 인스턴스를 직렬화 프록시로 변환
    - 바깥 클래스와 논리적으로 동일한 인스턴스를 반환하는 readResolve 메소드를 SerializationProxy 클래스에 추가
        - 역직렬화 시에 직렬화 시스템이 직렬화 프록시를 다시 바깥 클래스의 인스턴스로 변환하게 해줌

<br>

- **직렬화 프록시 패턴의 장점**
    - **일반 인스턴스를 만들 때와 똑같은 생성자, 정적 팩터리, 혹은 다른 메소드를 사용해 역직렬화된 인스턴스를 생성한다.**
        - 직렬화는 생성자를 이용하지 않고도 인스턴스를 생성
        - 역직렬화된 인스턴스가 해당 클래스의 불변식을 만족하는지 검사할 또 다른 수단을 강구하지 않아도 된다.
    - **방어적 복사처럼, 가짜 바이트 스트림 공격과 내부 필드 탈취 공격을 프록시 수준에서 차단해준다.**
        - 직렬화 프록시는 클래스의 필드를 final로 선언하여 불변으로 만들 수 있다.
...
- **직렬화 프록시 패턴의 한계**
    ...
    - 객체 그래프에 순환이 있는 클래스에 적용할 수 없다.
        - 직렬화 프록시만 가졌을 뿐 실제 객체는 아직 만들어진 것이 아님
    - 방어적 복사 때보다 느리다.
