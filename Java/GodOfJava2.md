...
### Serializable
    ...
    Serializable 인테페이스를 구현하면 JVM에서 해당 객체는 저장하거나 다른 서버로 전송할 수 있도록 해 준다.  

- Serializable을 구현해야 하는 상황:
    - 생성한 객체를 파일로 저장
    - 저장한 객체를 읽을 때
    - 객체를 다른 서버로 보낼 때
...
- **serialVersionUID**
    - 해당 객체의 버전을 명시하는 데 사용.
        - 서로 다른 서버에서 객체를 주고 받을 때 해당 객체가 같은지 다른지를 확인하는 값
        - 클래스 이름이 같더라도 ID가 다르면 다른 클래스로 인식
        - 같은 UID라하더라도 변수 개수나 타입 등이 다르면 다른 클래스로 인식 (예외상황은 발생하지 않지만 운영 상황에서 데이터가 꼬일 수 있기 때문에 절대 권장하지 않는다. 데이터가 바뀌면 serialVersionUID 값을 변경하는 습관 필요 or 개발툴에게 자동으로 생성되도록 맡긴다.)
    - static final long 형
    - 인터페이스 구현 후 별도로 지정 가능. 
    - 지정하지 않을 경우 자바 소스가 컴파일 될 때 자동으로 생성된다.

    *ObjectOutputStream/ObjectInputStream 클래스를 사용하여 객체를 저장, 읽을 수 있다.*  

- **transient** 예약어
    - 객체를 저장하거나 다른 JVM으로 보낼 때 transient로 선언한 변수는 Serializable 대상에서 제외된다. (저장 대상에서 제외)
    - 해당 객체를 생성한 JVM에서 이 변수를 사용하는 것엔 문제가 없다.
        > 문제상황: 패스워드 같은 중요한 변수가 저장되거나 전송된다면 보안상 큰 문제가 발생할 수 있다.
        > 해결: 보안상 중요한 변수나 꼭 저장해야 할 필요가 없는 변수에 대해 transient를 사용하여 선언을 한다.
...
- 자바 NIO란?
    - JDK 1.4부터 NIO 추가
    - 기존의 IO 보다 빠른 처리 속도
    - 스트림을 사용하지 않고 **Channel**과 **Buffer**를 사용한다.
        > *채널은 물건을 중간에서 처리하는 도매상, 버퍼는 도매상에서 물건을 사고 소비자에세 물건을 파는 소매상이라고 생각하면 된다.*

- Buffer
    - 버퍼는 CD 처럼 위치가 있다. 버퍼에 데이터를 담거나 읽는 작업을 수행하면 현재의 위치가 이동한 후 다음 위치에 있는 것을 바로 쓰거나 읽는다.
    - "0 <= mark(위치 저장) <= position(현재 버퍼위치) <= limit(읽거나 쓸 수 없는 첫 위치) <= capacity(버퍼 크기)"
